      ...
      <div className="max-w-[940px] mx-auto bg-white shadow-xl min-h-screen border-x border-gray-200 relative">
        ...
        <div className="absolute top-6 right-10 z-10">
          <Link to={isLoggedIn ? "/admin" : "/login"} className="group flex flex-col items-center gap-1">
            <div className="w-10 h-10 rounded-full bg-white/50 border border-gray-200 p-1 group-hover:bg-blue-50 transition-all overflow-hidden">
               <img src="https://images.rawpixel.com/image_png_800/cHJpdmF0ZS9sci9pbWFnZXMvd2Vic2l0ZS8yMDIzLTAxL3JtNjA5LXNvbGlkaWNvbi13LTAwMi1wLnBuZw.png" alt="User Profile" className="w-full h-full object-contain opacity-60 group-hover:opacity-100 transition-opacity" />
            </div>
        ...
        <header className="p-10 pt-16">
          <Link to="/" className="inline-block group">
            {settings.logo_url ? (
                <div className="mb-2"><img src={settings.logo_url} alt={settings.title} className="max-h-20 w-auto object-contain group-hover:opacity-80 transition-opacity" /></div>
            ) : (
                <h1 className="text-4xl font-black text-gray-900 leading-none mb-2 font-serif group-hover:text-blue-700 transition-colors">{settings.title}</h1>
            )}
          </Link>
          ...
          <div className="flex flex-wrap items-center">
            ...
            <div className="relative" ref={dropdownRef}>
                <button 
                  ...
                  className={`px-6 py-4 inline-block text-[13px] font-black uppercase tracking-widest transition-colors text-gray-300 hover:text-white hover:bg-[#333] ${isAboutOpen ? 'bg-[#333] text-white' : ''}`}
                >
                  About Us ▾
                </button>
                {isAboutOpen && (
                  <div className="absolute left-0 top-full w-48 bg-[#1d2327] z-[100] shadow-2xl border border-gray-800">
                    <Link 
                      ...
                      className="block px-6 py-4 text-[11px] font-black uppercase tracking-widest text-gray-300 hover:bg-blue-600 hover:text-white border-b border-white/5 transition-all"
                    >
                      Meet our team
                    </Link>
                    <Link 
                      to="/contact" 
                      onClick={() => setIsAboutOpen(false)}
                      className="block px-6 py-4 text-[11px] font-black uppercase tracking-widest text-gray-300 hover:bg-blue-600 hover:text-white transition-all"
                    >
                      Contact
                    </Link>
                  </div>
            ...
            {navPages.map((page, idx) => (
              <Link key={idx} to={`/post/${page.slug}`} className={getNavClass(`/post/${page.slug}`)}>{page.title}</Link>
            ))}
